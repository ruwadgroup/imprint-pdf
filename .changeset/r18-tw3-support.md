...
'@imprint-pdf/tailwind': minor
'@imprint-pdf/next': minor
'@imprint-pdf/vite': patch
---

Zero-install React 18 + Tailwind v3 support.
...
against a `@tailwind base/components/utilities` stub with the full class set as
`safelist`; v4 → unchanged `tw.compile()` path.
...
`tailwindcss` peer broadens to `^3.4.0 || ^4.0.0`; `postcss` is now an optional
peer (only required on v3). `react` peer broadens to `>=18.2.0 <20`. `next` peer
broadens to `^14.0.0 || ^15.0.0 || ^16.0.0`. `vite` peer broadens to
`^5.0.0 || ^6.0.0 || ^7.0.0`.
