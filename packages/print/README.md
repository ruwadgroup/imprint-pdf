...
Enterprise print-ready output for [Imprint](https://github.com/tamimbinhakim/imprint).

**License: Business Source License 1.1.** Reverts to Apache-2.0 after four
years. Commercial use requires a license — see [`LICENSING.md`](../../LICENSING.md).
...
- **PDF/X-4 and PDF/X-4p** output intent with embedded ICC profiles
  (Coated FOGRA39 / PSO Coated v3 by default; bring your own ICC file).
- **CMYK colour pipeline.** Tailwind colors convert from OKLCH/sRGB to CMYK
  via Little CMS (lcms2 compiled to WASM). Use `imprint:cmyk-[c_m_y_k]`
  Tailwind variants for explicit CMYK values.
- **Spot colors.** `imprint:spot-[Pantone-185-C]` and overprint via
  `imprint:overprint`.
- **Bleed, trim, crop, and registration marks.** `<Page bleed="3mm" marks="trim,registration">`.
