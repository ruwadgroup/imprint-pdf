...
WASM module instantiation dominates cold start (~30–60 ms). This is a fixed
cost independent of document complexity.
...
Typography is the dominant cost for text-heavy documents. Knuth–Plass runs
at ~1 ms per paragraph in practice.
...
Tree-shaking removes feature flags you don't import. An invoice-only client
that uses only Flexbox and skips the Oxide fallback is ~600 KB gzipped.
...
- Times use `performance.now()` around `renderToBuffer` calls. The first run
  in a process is excluded (V8 warm-up); reported numbers are median of 100
  runs on warm WASM.
- Cloudflare Worker numbers are from `wrangler dev` with `--local` flag and
  the Workers Unbound resource tier. Production edge may vary ±20%.
