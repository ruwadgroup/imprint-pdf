...
JSON schemas, not flowable React components. The migration is a conceptual shift,
...
pdfme's WYSIWYG designer and absolute-positioning model are genuinely better
for data-fill templates like labels, simple forms, and certificates where
designers (not engineers) own the layout. If that's your workflow, pdfme is
fine.
...
```json
{
  "schemas": [
    [
      { "type": "text", "name": "name", "position": { "x": 50, "y": 80 }, "width": 150, "height": 10 },
      { "type": "text", "name": "amount", "position": { "x": 150, "y": 120 }, "width": 50, "height": 10 }
    ]
  ]
}
```
